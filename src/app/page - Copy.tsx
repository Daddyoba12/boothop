      ...
      <nav className={`fixed top-0 z-50 w-full transition-all duration-300 ${scrolled ? 'border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur-md' : 'bg-transparent'}`}>
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          ...
          <div className="hidden items-center gap-1 md:flex">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href}
                className={`rounded-lg px-4 py-2 text-sm font-medium transition-all duration-300 ${scrolled ? 'text-slate-600 hover:bg-slate-100 hover:text-slate-900' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}>
                {link.label}
              </Link>
            ))}
          </div>
          ...
          <div className="hidden items-center gap-3 md:flex">
            <Link href="/login" className={`rounded-lg px-4 py-2 text-sm font-medium transition-all duration-300 ${scrolled ? 'text-slate-600 hover:bg-slate-100' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}>Log in</Link>
          </div>

          <button className={`rounded-lg p-2 md:hidden transition-all ${scrolled ? 'text-slate-700 hover:bg-slate-100' : 'text-white hover:bg-white/10'}`}
        ...
        {mobileOpen && (
          <div className="border-t border-slate-100 bg-white/95 shadow-lg backdrop-blur-md md:hidden animate-in slide-in-from-top duration-300">
            <div className="space-y-1 px-4 py-4">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)}
                  className="block rounded-lg px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors">{link.label}</Link>
              ))}
              <div className="flex flex-col gap-2 pt-3">
                <Link href="/login" className="block rounded-xl border border-slate-200 py-3 text-center text-sm font-medium text-slate-700 hover:bg-slate-50 transition-all">Log in</Link>
              </div>
            </div>
          </div>
        )}
...
white transition-all hover:shadow-xl hover:shadow-blue-500/40 hover:scale-105 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed">
      ...
      {/* ── POPULAR ROUTES ── */}
      <section className="relative bg-white px-4 py-16 md:py-24 overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-7xl">
          <div className="mb-10 text-center">
            <p className="mb-3 text-sm font-semibold uppercase tracking-widest bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">Global Network</p>
            <h2 className="mb-4 text-3xl font-bold bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900 bg-clip-text text-transparent md:text-4xl">Popular Routes</h2>
      ...
      {/* ── PRICING ── */}
      <section className="relative bg-slate-50 px-4 py-16 md:py-24 overflow-hidden">
        {/* Background decoration */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute -top-40 -right-40 w-96 h-96 bg-blue-100/50 rounded-full blur-3xl" />
          <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-emerald-100/50 rounded-full blur-3xl" />
        </div>
        
        <div className="relative z-10 mx-auto max-w-5xl">
          <div className="mb-10 text-center">
            <p className="mb-3 text-sm font-semibold uppercase tracking-widest bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">Transparent Pricing</p>
            <h2 className="mb-4 text-3xl font-bold bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900 bg-clip-text text-transparent md:text-4xl">Simple, honest fees</h2>
            <p className="text-base text-slate-500 md:text-lg">No hidden charges. Pay only when a delivery is confirmed.</p>
          </div>
          <div className="grid gap-6 md:grid-cols-2">
            {/* Hoopers Card */}
            <div className="group rounded-3xl border border-slate-200 bg-white/80 backdrop-blur-sm p-6 shadow-lg md:p-8 transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:shadow-emerald-500/20 hover:border-emerald-300 animate-in fade-in slide-in-from-left duration-700">
              <div className="mb-6 flex items-center gap-3">
                ...
                <div><p className="font-bold bg-gradient-to-r from-emerald-700 to-teal-700 bg-clip-text text-transparent">Hoopers (Senders)</p><p className="text-sm text-slate-500">People sending items</p></div>
              </div>
              <div className="mb-3 flex items-baseline gap-1">
                <span className="text-4xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent md:text-5xl">+3%</span>
              ...
              <div className="rounded-xl bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-200 p-4 text-sm text-slate-600 backdrop-blur-sm">Example: Agree £100 → you pay <strong className="text-emerald-700">£103</strong> total.</div>
            </div>
